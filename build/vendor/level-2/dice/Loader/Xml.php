 ...
 * @version 3.0 */
namespace PcLocations_001\Dice\Loader;

class Xml
{
    private function getComponent(\PcLocations_001\SimpleXmlElement $element, $forceInstance = \false)
    {
        if ($forceInstance) {
            return [\PcLocations_001\Dice\Dice::INSTANCE => (string) $element];
        } else {
            if ($element->instance) {
                return [\PcLocations_001\Dice\Dice::INSTANCE => (string) $element->instance];
            } else {
                return (string) $element;
            }
        }
    }
    private function loadV1(\PcLocations_001\SimpleXmlElement $xml, \PcLocations_001\Dice\Dice $dice)
    {
        $rules = [];
    ...
    private function loadV2(\PcLocations_001\SimpleXmlElement $xml, \PcLocations_001\Dice\Dice $dice)
    {
        $rules = [];
        foreach ($xml as $key => $value) {
            $rule = $dice->getRule((string) $value->name);
    ...
    public function load($xml, \PcLocations_001\Dice\Dice $dice = null, $displayWarning = \true)
    {
        if ($displayWarning) {
        ...
        if ($dice === null) {
            $dice = new \PcLocations_001\Dice\Dice();
        }
        if (!$xml instanceof \PcLocations_001\SimpleXmlElement) {
